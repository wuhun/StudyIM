...
public class PhotoUtils {
    ...
    public void takePicture(Activity activity) {
        try {
            //每次选择图片吧之前的图片删除
            WuhunFileTool.deleteFileForUri(buildUri());
...
//        if (WuhunFileTool.sdCardIsAvailable()) {
            File file = MyApplication.getCameraPifFile(CROP_FILE_NAME);
...
//        }
    ...
    public void onActivityResult(Activity activity, int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            //拍照
            case INTENT_TAKE:
                TestLog.i("显示图片：" + buildUri().getPath());
                if (new File(buildUri().getPath()).exists()) {
                    if (corp(activity, buildUri())) {
                        return;
                    }
            ...
            case INTENT_CROP:
                if (resultCode == Activity.RESULT_OK && new File(buildUri().getPath()).exists()) {
                    onPhotoResultListener.onPhotoResult(buildUri());
                }
                break;
        }
    }
